...
\begin{definition}[Autonomous System]
	An \bld{autonomous Lagrangian system} is defined to be a tuple $(M,L)$ consitsing of a smooth manifold $M$ and a function $L \in C^\infty(M)$.
\end{definition}
...
\begin{definition}[Tautological Form]
	...
	Let $M$ be a smooth manifold. The \bld{tautological form on $T^*M$}\index{Form!tautological}, denoted by $\alpha$, is the form  $\alpha \in \upOmega^1(T^*M)$ defined locally by
	\begin{equation*}
		\alpha := \xi_idx^i,
	\end{equation*}
