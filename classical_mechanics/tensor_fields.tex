...
\begin{definition}[Tensor Field]
	Let $M$ be a smooth manifold and $k,l \in \mathbb{N}$. A \bld{smooth tensor field of type $(k,l)$ on $M$} is defined to be a section of $\pi : T^{(k,l)}TM \to M$. The space of all smooth tensor fields of type $(k,l)$ on $M$ is denoted by $\mathcal{T}^{k,l}(M) := \upGamma\del[1]{T^{(k,l)}TM}$.
\end{definition}
...
\begin{proposition}[{Tensor Field Characterisation Lemma \cite[318]{lee:smooth_manifolds:2013}}]
